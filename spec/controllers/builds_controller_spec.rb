...
describe BuildsController, '#create' do
  ...
  context 'when number of changed files is below the threshold' do
    it 'enqueues small build job' do
      allow(SmallBuildJob).to receive(:perform_now)
      payload_data = File.read(
        'spec/support/fixtures/pull_request_opened_event.json'
      )
      ...
      expect(SmallBuildJob).to have_received(:perform_now).with(
        payload.build_data
      )
    end
  end
  ...
  context 'when number of changed files is at the threshold or above' do
    it 'enqueues large build job' do
      allow(LargeBuildJob).to receive(:perform_now)
      payload_data = File.read(
      ...
      expect(LargeBuildJob).to have_received(:perform_now).with(
        payload.build_data
      )
    end
  end
  ...
  context "when payload is not for pull request" do
    it "does not schedule a job" do
      ...
      expect(LargeBuildJob).not_to have_received(:perform_now)
      expect(SmallBuildJob).not_to have_received(:perform_now)
    end
  end
  ...
  context "when payload is not nested under a key" do
    it "enqueues a job" do
      ...
      expect(SmallBuildJob).to have_received(:perform_now).
